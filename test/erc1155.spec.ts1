...
contract('ERC1155Mintable - tests all core 1155 functionality.', (accounts) => {
  ...
  it('safeTransferFrom throws with no balance', async () => {
    await expectThrow(
      mainContract.safeTransferFrom(user2, user1, hammerId, 1, web3.utils.fromAscii(''), {
        from: user2,
      })
    );
  });
  ...
  it('safeTransferFrom throws with invalid id', async () => {
    await expectThrow(
      mainContract.safeTransferFrom(user2, user1, 32, 1, web3.utils.fromAscii(''), { from: user2 })
    );
  });

  it('safeTransferFrom throws with no approval', async () => {
    await expectThrow(
      mainContract.safeTransferFrom(user1, user2, hammerId, 1, web3.utils.fromAscii(''), {
        from: user2,
      })
    );
  });
  ...
  it('safeTransferFrom throws when exceeding balance', async () => {
    await expectThrow(
      mainContract.safeTransferFrom(user1, user2, hammerId, 6, web3.utils.fromAscii(''), {
        from: user1,
      })
    );
  });
  ...
  it('safeBatchTransferFrom throws when exceeding balance', async () => {
    await expectThrow(
      mainContract.safeBatchTransferFrom(user1, user2, idSet, [6, 1, 1], web3.utils.fromAscii(''), {
        from: user1,
      })
    );
  });
  ...
  it('safeBatchTransferFrom ok with valid receiver contract reply', async () => {
    ...
    await testSafeBatchTransferFrom(
      ...
      quantities,
      web3.utils.fromAscii(''),
      'testSafeBatchTransferFrom receiver'
    );
  });
